...
func TestRunningMultipleSchedules(t *testing.T) {
	...
	cron.AddFunc("0 0 0 31 12 ?", func(time.Time) {})
	cron.AddFunc("* * * * * ?", func(time.Time) { wg.Done() })
	cron.Schedule(FuncJob(Every(time.Minute), func(time.Time) {}))
	cron.Schedule(FuncJob(Every(time.Second), func(time.Time) { wg.Done() }))
	cron.Schedule(FuncJob(Every(time.Hour), func(time.Time) {}))
...
func (t testJob) Schedule() Schedule {
	return t.schedule
}
...
func TestInvalidJobSpec(t *testing.T) {
	cron := New()
	_, err := cron.AddFunc("this will not parse", nil)
	if err == nil {
...
func TestJob(t *testing.T) {
	...
	cron := newWithSeconds()
	cron.AddJob(testJob{parseCustom("0 0 0 30 Feb ?", cron.parser), wg, "job0"})
	cron.AddJob(testJob{parseCustom("0 0 0 1 1 ?", cron.parser), wg, "job1"})
	job2, _ := cron.AddJob(testJob{parseCustom("* * * * * ?", cron.parser), wg, "job2"})
	cron.AddJob(testJob{parseCustom("1 0 0 1 1 ?", cron.parser), wg, "job3"})
	cron.Schedule(testJob{Every(5*time.Second + 5*time.Nanosecond), wg, "job4"})
	job5 := cron.Schedule(testJob{Every(5 * time.Minute), wg, "job5"})
...
func TestScheduleAfterRemoval(t *testing.T) {
	...
	cron := newWithSeconds()
	hourJob := cron.Schedule(FuncJob(Every(time.Hour), func(time.Time) {}))
	cron.Schedule(FuncJob(Every(time.Second), func(time.Time) {
		mu.Lock()
		defer mu.Unlock()
		switch calls {
		case 0:
...
func TestJobWithZeroTimeDoesNotRun(t *testing.T) {
	...
	var calls int64
	cron.AddFunc("* * * * * *", func(time.Time) { atomic.AddInt64(&calls, 1) })
	cron.Schedule(FuncJob(new(ZeroSchedule), func(time.Time) { t.Error("expected zero task will not run") }))
	cron.Start()
	defer cron.Stop()
	<-time.After(OneSecond)
	if atomic.LoadInt64(&calls) != 1 {
...
func parse(standardSpec string) Schedule {
	s, err := standardParser.Parse(standardSpec)
	if err != nil {
		panic(err)
	}
	return s
...
func parseCustom(standardSpec string, parser ScheduleParser) Schedule {
	s, err := parser.Parse(standardSpec)
	if err != nil {
		panic(err)
	}
	return s
